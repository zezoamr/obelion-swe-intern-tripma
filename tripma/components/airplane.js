...
export default function Airplane({ seatsData, dividerLocations }) {
    const [selectedSeat, setSelectedSeat] = useState(null);

    const { maxSeatsInRow, totalRows } = useMemo(() => {
        let maxSeats = 0;
        seatsData.forEach(row => {
            maxSeats = Math.max(maxSeats, row.length);
        });
        return { maxSeatsInRow: maxSeats, totalRows: seatsData.length };
    }, [seatsData]);

    return (
        <div className={styles.airplaneContainer} style={{
